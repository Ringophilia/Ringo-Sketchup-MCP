...
软件版本为 `1.3.0`；协议版本单独使用 major 3。协议 major 3 内的客户端通过 capabilities 协商；只有无法互相理解的协议主版本才明确拒绝连接，提示更新客户端或扩展。
...
1.2.0 迁移：首次修改前读取 model_get_info，后续修改自动绑定该模型；切换模型或编辑上下文后重新读取。分页默认不统计完整总数，需要时传 include_total:true。

1.3.0 支持多实例 profile。每个 profile 使用独立配置和端口；同一 SketchUp 进程通过扩展菜单选择 profile。多个实例的 `model_id`、实体路径和快照不能互换。
