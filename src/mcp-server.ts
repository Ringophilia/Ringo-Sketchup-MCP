...
const modelRef = {model_id: z.string().optional()};
...
{name:'camera_set',method:'camera.set',description:'Set camera and optionally frame an entity or the whole model.',schema:{...modelRef,eye:vec.optional(),target:vec.optional(),up:vec.optional(),perspective:z.boolean().optional(),fov:n.min(1).max(120).optional(),entity_id:id.optional(),zoom_extents:z.boolean().optional(),unit}},
...
export function createServer(client = new SketchupClient()) {
  ...
  for (const def of defs) server.registerTool(def.name, {
    ...
    try {
      ...
      const requestArgs = def.method === 'batch.run' ? validateBatchArgs(args) : args;
      ...
      if (def.method === 'view.export' && result && typeof result === 'object' && 'data' in result) {
        ...
        if (path && (ext === 'png' || ext === 'jpg' || ext === 'jpeg')) {
          try {
            ...
            if (bytes.byteLength <= 4 * 1024 * 1024) content.push({type:'image',data:bytes.toString('base64'),mimeType:ext === 'png' ? 'image/png' : 'image/jpeg'});
